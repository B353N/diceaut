...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <h1 class="h3 mb-2 text-gray-800">
            {{ clean(trans('niva-backend.section_4_services'), ['Attr.EnableID' => true]) }}</h1>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <a href="{{ route('home-setting.edit') . '?language=' . request()->input('language') }}"
                                class="btn btn-primary btn-back">Върни се в начало</a>
                            <a href="{{ route('service.create') . '?language=' . request()->input('language') }}"
                                class="btn btn-primary btn-back">Създай услуга</a>
                        </div>
                    ...
                    <form action="{{ route('delete.service') }}" method="POST" class="form-inline">
                        ...
                        <div class="form-group">
                            <input type="submit" name="delete_all" class="btn btn-primary" value="Изпълни">
                        </div>
                                    ...
                                    @foreach ($services as $service)
                                        <tr>
                                            <td><input class="checkboxes" type="checkbox" name="checkbox_array[]"
                                                    value="{{ $service->id }}"></td>
                                            <td><img height="100"
                                                    src="{{ $service->photo ? '/images/media/' . $service->photo->file : '/public/img/200x200.png' }}"
                                                    alt="">
                                                <p class="mb-0 mt-2"><a
                                                        href="{{ route('service.edit', $service->id) . '?language=' . request()->input('language') }}">{{ clean(trans('niva-backend.edit'), ['Attr.EnableID' => true]) }}</a>
                                                </p>
                                            </td>
