...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <!-- Page Heading -->
        <h1 class="h3 mb-2 text-gray-800">{{ clean(trans('niva-backend.all_languages'), ['Attr.EnableID' => true]) }}
        </h1>
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">
                    {{ clean(trans('niva-backend.all_languages'), ['Attr.EnableID' => true]) }}</h6>
            </div>
            <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <a href="{{ route('language.create') }}"
                                class="btn btn-primary btn-back">{{ clean(trans('niva-backend.create_language'), ['Attr.EnableID' => true]) }}</a>
                        </div>
                        ...
                        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ clean(trans('niva-backend.code_lang'), ['Attr.EnableID' => true]) }}</th>
                                    <th>{{ clean(trans('niva-backend.is_default'), ['Attr.EnableID' => true]) }}
                                    </th>
                                    <th>{{ clean(trans('niva-backend.ltr_rtl'), ['Attr.EnableID' => true]) }}</th>
                                </tr>
                            </thead>
                            <tfoot>
                                <tr>
                                        ...
                                        <tr>
                                            ...
                                            <td class="language-name" data-label="Name">
                                                <div class="float-left-language-name">
                                                    <p>{{ $language->name }}</p>
                                                    <a
                                                        href="{{ route('language.edit', $language->id) }}">{{ clean(trans('niva-backend.edit'), ['Attr.EnableID' => true]) }}</a>
                                                </div>
                                            </td>
