...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <!-- Page Heading -->
        <h1 class="h3 mb-2 text-gray-800">{{ clean(trans('niva-backend.all_sliders'), ['Attr.EnableID' => true]) }}
        </h1>
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">
                    {{ clean(trans('niva-backend.all_sliders'), ['Attr.EnableID' => true]) }}</h6>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            ...
                            <a href="{{ route('slider.create') . '?language=' . request()->input('language') }}"
                                class="btn btn-primary btn-back">{{ clean(trans('niva-backend.create_slider'), ['Attr.EnableID' => true]) }}</a>
                        </div>
                        ...
                        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ clean(trans('niva-backend.heading_1'), ['Attr.EnableID' => true]) }}</th>
                                    <th>{{ clean(trans('niva-backend.heading_2'), ['Attr.EnableID' => true]) }}</th>
                                    <th>{{ clean(trans('niva-backend.button_text'), ['Attr.EnableID' => true]) }}
                                    </th>
                                    <th>{{ clean(trans('niva-backend.button_link'), ['Attr.EnableID' => true]) }}
                                    </th>
                                    ...
                                    @foreach ($sliders as $key => $slider)
                                        <tr>
                                            <td><input class="checkboxes" type="checkbox" name="checkbox_array[]"
                                                    value="{{ $slider->id }}"></td>
                                            <td><img height="100"
                                                    src="{{ $slider->photo ? '/images/media/' . $slider->photo->file : '/public/img/200x200.png' }}"
                                                    alt="">
                                                <p class="mb-0 mt-2"><a
                                                        href="{{ route('slider.edit', $slider->id) . '?language=' . request()->input('language') }}">{{ clean(trans('niva-backend.edit'), ['Attr.EnableID' => true]) }}</a>
                                                </p>
                                            </td>
