...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <!-- Page Heading -->
        <h1 class="h3 mb-2 text-gray-800">{{ clean(trans('niva-backend.section_7_blog'), ['Attr.EnableID' => true]) }}
        </h1>
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            @if (Auth::user()->role->name == 'administrator')
                                ...
                                <a href="{{ route('blog-setting.edit') . '?language=' . request()->input('language') }}"
                                    class="btn btn-primary btn-back">{{ clean(trans('niva-backend.back_blogpage'), ['Attr.EnableID' => true]) }}</a>
                            @endif
                            <a href="{{ route('post.create') . '?language=' . request()->input('language') }}"
                                class="btn btn-primary btn-back">{{ clean(trans('niva-backend.create_article'), ['Attr.EnableID' => true]) }}</a>
                        </div>
                        ...
                        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">
                                        {{ clean(trans('niva-backend.id'), ['Attr.EnableID' => true]) }}</th>
                                    <th scope="col">
                                    ...
                                    @foreach ($posts as $post)
                                        <tr>
                                            ...
                                            <td data-label="Photo"><img height="50"
                                                    ...
                                                    alt="">
                                                <p class="mb-0 mt-2"><a
                                                        href="{{ route('post.edit', $post->id) . '?language=' . request()->input('language') }}">{{ clean(trans('niva-backend.edit'), ['Attr.EnableID' => true]) }}</a>
                                                </p>
                                            </td>
